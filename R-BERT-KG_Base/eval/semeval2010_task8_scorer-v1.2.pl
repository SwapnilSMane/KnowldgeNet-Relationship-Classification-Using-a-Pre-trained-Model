...
my (%confMatrix12way, %confMatrix10wayNoDir, %confMatrix10wayWithDir) = ();
my (%idsProposed, %idsAnswer) = ();
my (%allLabels12waylAnswer, %allLabels10wayAnswer) = ();
my (%allLabels12wayProposed, %allLabels10wayNoDirProposed, %allLabels10wayWithDirProposed) = ();
...
foreach my $id (keys %idsProposed) {
	...
	### 2.2. Update the 12-way confusion matrix
	my $labelProposed = $idsProposed{$id};
	my $labelAnswer   = $idsAnswer{$id};
	$confMatrix12way{$labelProposed}{$labelAnswer}++;
	$allLabels12wayProposed{$labelProposed}++;
...
foreach my $id (keys %idsAnswer) {
	...
	### 3.1. Update the 12-way answer distribution
	my $labelAnswer = $idsAnswer{$id};
	$allLabels12waylAnswer{$labelAnswer}++;
...
foreach my $labelProposed (sort keys %allLabels12wayProposed) {
	if (!defined($allLabels12waylAnswer{$labelProposed})) {
		print "!!!WARNING!!! The proposed file contains $allLabels12wayProposed{$labelProposed} label(s) of type '$labelProposed', which is NOT present in the key file.\n\n";
	}
}

### 4. 12-way evaluation with directionality
print "<<< (2*5+2)-WAY EVALUATION (USING DIRECTIONALITY)>>>:\n\n";
&evaluate(\%confMatrix12way, \%allLabels12wayProposed, \%allLabels12waylAnswer, $totalProposed, $totalAnswer, 0);

### 5. Evaluate without directionality
print "<<< (5+2)-WAY EVALUATION IGNORING DIRECTIONALITY >>>:\n\n";
...
print "<<< (5+2)-WAY EVALUATION TAKING DIRECTIONALITY INTO ACCOUNT -- OFFICIAL >>>:\n\n";
...
printf "<<< The official score is (5+2)-way evaluation with directionality taken into account: macro-averaged F1 = %0.2f%s >>>\n", $officialScore, '%';
...
sub getIDandLabel() {
	...
	return ($id, $label)
    if (($label eq 'EMPLOYEE_OR_MEMBER_OF(e1,e2)')       || ($label eq 'EMPLOYEE_OR_MEMBER_OF(e2,e1)')       ||
		($label eq 'PLACE_OF_RESIDENCE(e1,e2)')    || ($label eq 'PLACE_OF_RESIDENCE(e2,e1)')    ||
		($label eq 'EDUCATED_AT(e1,e2)')  || ($label eq 'EDUCATED_AT(e2,e1)')  ||
		($label eq 'PLACE_OF_BIRTH(e1,e2)') || ($label eq 'PLACE_OF_BIRTH(e2,e1)') ||
		($label eq 'Entity-Origin(e1,e2)')      || ($label eq 'Entity-Origin(e2,e1)')      ||
		($label eq 'DATE_OF_BIRTH(e1,e2)')  || ($label eq 'DATE_OF_BIRTH(e2,e1)')  ||
		($label eq 'NATIONALITY(e1,e2)')  || ($label eq 'NATIONALITY(e2,e1)'));
